 ...
 */
class Following(spark: SparkSession) {
  
  import spark.implicits._
  
  /**
   * @param annot Dataset of AQAnnotations, the ones we will be using to look for following sibling annotations. 
   * @param anchor Dataset of AQAnnotations  starting point for using to look for following sibling annotations (use the endOffset and docId).
   * @param container Dataset of AQAnnotations to use when requiring the following sibling annotations to be contained in a specific annotation.
  ...
  def apply(annot: Dataset[AQAnnotation], 
    ...
    var results = annot.as("L").joinWith(anchor.as("R"),
                               ...
                               .mapGroups( (k,v) => {
                                  ...
                                  (key,lb.sortWith(_.startOffset < _.startOffset).toArray.slice(0,cnt))
                               })
